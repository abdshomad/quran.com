...
=head2 $self->capture_match( $c, $action, \@captures )

Internally test if capture args match against expressions supplied through :Match attribute.

e.g. :CaptureArgs(1) :Match('^\w+$')
e.g. :CaptureArgs(2) :Match('^\w+$','\d+')
...
sub capture_match {
	...
	if (my $match = $action->attributes->{Match}) {
		$match = [ map { qr/$_/ } split /'\s*,\s*'|"\s*,\s*"/, $match->[0] ];

		return 0 unless scalar @{ $match } >= scalar @{ $captures };
		for (@{ $captures }) { return 0 unless $_ =~ shift @{ $match }; }
                    ...
                    (!$best_action                                 ||
                     ...
                     ($#$action_parts == $#{$best_action->{parts}} &&
                      ...
                      ($n_pathparts > $best_action->{n_pathparts}  ||
                       $n_pathparts == $best_action->{n_pathparts} &&
                       $self->capture_match($c, $action, \@captures))))) {

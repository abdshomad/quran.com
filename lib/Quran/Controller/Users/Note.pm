...
package Quran::Controller::Users::Note;
use Moose;
use namespace::autoclean;

BEGIN { extends 'Catalyst::Controller' }

sub base :Chained('/users/base') :PathPart('note') :CaptureArgs(0) {}
sub index :Chained('base') :PathPart('') :Args(0) {
	...
	if ($c->request->method eq 'POST' and $c->user_exists and defined $p->{ayah} and $p->{ayah} =~ /^[\d]+:[\d]+$/ and defined $p->{body}) {
		...
		$c->response->body('Matched Quran::Controller::Users::Note in Users::Note.');
	}
}

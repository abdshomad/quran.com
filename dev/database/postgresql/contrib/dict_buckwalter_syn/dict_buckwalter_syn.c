 ...
 *
 * IDENTIFICATION
 *	  $PostgreSQL: pgsql/src/backend/tsearch/dict_buckwalter_syn.c,v 1.13 2010/02/26 02:01:05 momjian Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
PG_FUNCTION_INFO_V1(dbuckwalter_syn_init);
Datum		dbuckwalter_syn_init(PG_FUNCTION_ARGS);

PG_FUNCTION_INFO_V1(dbuckwalter_syn_lexize);
Datum		dbuckwalter_syn_lexize(PG_FUNCTION_ARGS);


Datum
dbuckwalter_syn_init(PG_FUNCTION_ARGS)
{
...
Datum
dbuckwalter_syn_lexize(PG_FUNCTION_ARGS)
{

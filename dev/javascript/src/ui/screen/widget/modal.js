(function( Quran, jQuery ) {
	...
	Quran.ui.widget.modal = function(quran) {
		var self = this;
	};
	Quran.ui.widget.modal.prototype = {
		test: function() {
			var self = this;

			console.time('modal test');

			self.modal = jQuery.tmpl('quran.account.sign-in').modal({
				minWidth: 620,
				title: 'Sign In',
				footer: {
					status: 'Sign In',
				...
				sections: {
					finish: {
						content: '<h2>finished</h2><p>finished</p>',
						footer: {
							status: 'Signed In',
							buttons: [{
								label: 'Reload Page',
								click: function() {
									console.debug('finish', 'context', this, 'arguments', arguments);
									this.open('main');
								}
							}]
						}

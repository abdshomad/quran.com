...
//   - Length: # Ayahs
//   - Length: # Words
// - Add config group with interface preferences:
//   - Right side panel or left
//   - Fixed interface or scroll
	...
	Quran.ui.side.navMenu.prototype = {
		...
		_selectResourceCallback: function(result) {
			...
			if (result.saved) {
				...
				setTimeout(function() {
					quran.run('selectContent', result);
				}, 100);

				quran.change({ key: self._last_key, force: true, anchor: true }, 'selectContent');
			}
		},
		_selectQuranHandler: function(event, data) {
			...
			jQuery.each(self.inputData.keys, function(index, key) {
				...
				if (!self.inputData.cached)
					self.inputData.cached = quran.data('content.cached'); // quick hack/bug fix

				if (self.inputData.cached[key] && self.inputData.cached[key].quran[type] && (type != 'text' || self.inputData.cached[key].quran[type][value]))
					jQuery('#ayah-'+ key.split(/:/).join('-')).find('.quran .content p').filter('.'+ type + ( type == 'text' ? '.'+ value : '' )).removeClass('hidden').siblings().addClass('hidden');
			});
			...
			if (result.saved) {
				if (result.content && result.content.quran) {
					jQuery.each(result.content.quran, function (key, data) {
						var container = jQuery('#ayah-'+ key.split(/:/).join('-')), appendTo = container.find('> section.quran > div.content'), content;
						...
						if (content)
							content.appendTo(appendTo).siblings().addClass('hidden');
					});
				}
		...
		_selectSurah: function() {
			var self = this,
				surah = quran.data('view') == 'main' ? quran.page('surah') : null,
